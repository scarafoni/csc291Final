...
;   object moves -- but this requires inference in general, and
...
(defparameter *plan* 
;~~~~~~~~~~~~
  (make-array *n-agents*)); shows the currently best sequence of future actions

(defparameter *states* 
;~~~~~~~~~~~~~~
  (make-array *n-agents*)); *states*[j] is a sequence of states (where each 
                          ; state is a set of ground predications), starting 
                          ; at present, corresponding to *plan*[j];

(defparameter *here* 
;~~~~~~~~~~~~~
  (make-array *n-agents*)); each agent has a different "here"

(defparameter *inference-limit* 2) ; depth of forward inference;
;~~~~~~~~~~~~~~~~~~~~~~~~~~

(defparameter *curr-state-node* 
   (make-array *n-agents*)); each agent has a different knowledge state
(dotimes (j *n-agents*)
   (setf (aref *curr-state-node* j) 
         (make-hash-table :test #'equal)))

(defparameter *real-history* nil) ; the sequence of actions (with parameter values)
;~~~~~~~~~~~~~~~~~~~~~~~~~~ ; and events so far in the world

(defparameter *AG-history* 
;~~~~~~~~~~~~~~~~~~
   (make-array *n-agents*)); the sequence of actions (with parameter values)
                           ; taken so far from the agent's perspective

(defparameter *operators* nil) ; names of the available operators;
;~~~~~~~~~~~~~~~~~~~~~~  ; must be set by the user

(defparameter *search-beam* nil) ; list of items, each of form (i . op-names),
...
;; ;~~~~~~~~~~~~~~~~~~ THIS VERSION MALFUNCTIONS AFTER COMPILATION
;;                     BECAUSE OF AN ESOTERIC COMPILER BUG

(defun unionf (x y)
; ~~~~~~~~~~~~~~~~~
 (if (and (listp x) (listp y))
     (remove-duplicates (append x y) :test 'equal)
     (if (equal x y) x nil) ))
...
;
; Daphne: Revised 10/09/2012 to handle new representation of terms
; Daphne: Revised Dec. 2009 to handle new hashtable representation of wff-htable
...
; 'terms' are all the (not necessarily distinct) terms occurring in 
; fact-htable, precomputed for convenience.
;
...
(defun all-bindings-of-goals-to-fact-htable1 (goals fact-htable terms); Revised 
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   Dec. 2009 by Daphne Liu
; Find all unifiers of the variables occurring in goals (a set
; of +ve & -ve literals, possibly containing variables) obtainable
...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
; ff is the fact hashtable to be consulted, & we're also given the terms in it
	(if (poslit g)
...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        (let* ((shortest-leng 0) hash-value hash-leng
               ...
               (keys 
                 (generate_allkeys_from_hashkey (convert_pred_to_hashkey g))
               )
              ...
              (dotimes (i keys-leng)
                 ...
                 (when (not (null hash-value))
                       ...
                       (when (and (> hash-leng 0) 
                                  (or (> shortest-leng hash-leng) 
                                      (null first-non-null-hash-value-reached)))
 ...
 ); end of possible-positive-unifiers
...
;
 ...
 (defstruct state-node    ; a node in a tree of states generated
                          ; by forward search;
     terms                ; a list of (count unique_term) pairs in the wffs in wff-htable        
...
(defun chain-forward (agent state-node search-beams)
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
; sequence becomes available as *states*[j], where j is the index
; of the agent and a state is represented as a set of ground 
; predications).
;
...
;
 ...
 (let* ((j (gethash agent *agent-goal-indices*))
        (state-node-name (state-node-name state-node))
        (wff-htable (state-node-wff-htable state-node))
        (children (state-node-children state-node)); already-generated
        (operators (state-node-operators state-node)); already used to
                                                     ; generate children
   ...
   (when extra-ops
      ...
      (setq action-state-pairs
         (apply #'append
             (mapcar 
                #'(lambda (o) 
                     (all-instances-of-operator agent o state-node-name))
                                               ;^^^^^ for multiagent worlds
                extra-ops
             )
         )
      )
   ...
   (dolist (pair action-state-pairs); search forward recursively
      (chain-forward agent (eval (cdr pair)) (cdr search-beams)) 
   )
   ...
   (setq children ; merge new actions into them
	...
	(when children
	  ...
	  (while (and (< index (length children)) 
                      (= max-value (inclusive-value (nth index children))))
   ...
   (when children
      ...
      ; reset *plan*[j]
      (setf (aref *plan* j) (leftmost-action-sequence (car children))) 
      (setf (aref *states* j) (leftmost-state-sequence (car children)))
   )
...
(defun all-instances-of-operator (agent op-name state-node-name); agent added Oct 2012
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~; Revised Dec. 2009 by Daphne Liu
; Return all instances (with the given agent) of the operator named by
; 'op-name', paired with the names of the resulting state-nodess, that
; can be formed by binding the preconds of the operator to the wffs
...
;
 (let* ((op (eval op-name))
        ...
        (preconds (op-preconds op)); uninstantiated
        (wff-terms (state-node-terms state-node))
        (wff-htable (state-node-wff-htable state-node))
        bindings
        agent-var ; for multi-agent worlds
        agent-binding ; for multi-agent worlds
        instances
        state-nodes 
       )  
   
       ; IF THE GIVEN AGENT IS *NOT* 'AG, ASSUME WE ARE IN A MULTI-AGENT WORLD,
       ; and that all actions have an agent as first parameter. Therefore,
       ; in that case specialize the preconds so that occurrences of the first
       ; parameter of op in them are set to the value of 'agent'. -LKS, Oct'12
       (when (and agent (not (eq agent 'AG))); multi-agent world
          (setq agent-var (first (op-pars op))) 
          ; (format t "~%## AGENT-VAR = ~s~%" agent-var); DEBUG
          (setq preconds (subst agent agent-var preconds))
       ...
       ; In a multiagent world, add the agent binding to each of the bindings 
       ; found -LKS
       (when agent-var; non-nil for multi-agent worlds only
          (setq agent-binding (cons agent-var agent))
          ; (format t "~%## AGENT-BINDING = ~s~%" agent-binding); DEBUG
          (setq bindings
               (mapcar #'(lambda (x) (cons agent-binding x)) bindings)))
       ; (format t "~%## BINDINGS = ~s~%" bindings); DEBUG
  ...
  (let* ((action (eval action-name))
        ...
        (setq new-forward-value      
              (expected-rewards old-wff-htable))
               ; expected future rewards/costs, for
               ; action sequences (& their effects)  
		...
		(setq new-terms 
                   (remove_term_list_from_term_list (remove_list_of_tuples_from_hashtable 
                              deletions new-wff-htable 'T) new-terms))
		(setq new-terms (merge_term_list_with_term_list (add_list_of_tuples_to_hashtable 
                                           additions new-wff-htable 'T)
                                        new-terms))

        ;(setq new-wff-htable 
        ;  (append additions (set-differencef new-wff-htable deletions)))
...
; Daphne: Revised 10/9/2012 to handle new representation of terms
; Daphne: Revised Dec. 2009 to handle new hashtable representation of wff-htable 
(defun initialize-state-node (j)
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~; Multiagents, Oct 2/12. -LKS
; This augments the world state (*world-facts*) with inferences
; from *general-facts*, and then sets up the jth agent's initial 
; state-node, creating a new state-node name and providing its value 
; via `make-state-node'. This value also becomes *curr-state-node*[j].
...
; "non-occluded" facts (that should be evident to the jth agent)
; about entities located at *here*[j] (the point at which the agent
...
;
	(let*    (local-facts local-facts-terms-triple implied-facts
          	 (new-state-node-name (gensym "STATE-NODE"))
                 (agent (aref *agent-array* j)) new-terms
          	 (location (find-location agent *world-facts*))
       		)
		...
		; Check for predefined roadmap knowledge.
       	...
       	; Check for presence of agent at *here*[j].
       	(when (null location)
           (return-from initialize-state-node
             (concatenate 'string
               "** You need to do a place-object for "
               (string agent) " before initializing")))      

       (setf (aref *visited-places* j) (list location))
       (setf (aref *visited-objects* j) nil) ; updated in facts-evident-to function
       ...
       ; [**One inefficiency for multiple agents here is that if we
       ; initialize all agents before taking any action, we will repeat
       ; the same inference process each time.]
       (setq implied-facts
       ...
       (setq local-facts-terms-triple 
           (facts-evident-to agent *world-facts* 'T)
       )
;(format t "~% *world-facts*: ~A~%" *world-facts*)       
       (setq local-facts (first local-facts-terms-triple))
;(format t "~% local-facts: ~A~%" local-facts)  
;(format t "~%~% ~A  local-facts list: ~A~%~%" (length (third local-facts-terms-triple)) (third local-facts-terms-triple))    

       (setq new-terms (second local-facts-terms-triple))
;(format t "~% new-terms 1: ~A~%" new-terms)   
       ; Add any extra initial knowledge of the agent.
       (setq new-terms (merge_term_list_with_term_list new-terms 
       		(add_htable_to_hashtable 
                  (aref *extra-initial-knowledge* j) local-facts 'T)
       ))
;(format t "~% new-terms 2: ~A~%" new-terms)      

		;Unless we want the agent to be omiscient of all adjacency info
		;beyond just its neighbors', we shouldn't add in *roadmap-knoweldge*.
		;Note that *roadmap-knowledge* has knowledge of only the forms
		;(is_adjacent_to ...) and (road ...).    
(setq new-terms (merge_term_list_with_term_list new-terms 
			(add_htable_to_hashtable *roadmap-knowledge* local-facts 'T)
       ))
;(format t "~% new-terms 3: ~A~%" (apply '+ (mapcar #'first new-terms)))

		
		(setq new-terms (merge_term_list_with_term_list new-terms
			(add_htable_to_hashtable 
	                    (all-inferences local-facts 
	                         *general-knowledge* *inference-limit*) local-facts 'T)
	        ))
;(format t "~% new-terms 4: ~A~%" (apply '+ (mapcar #'first new-terms)))
		    
       ; Create a corresponding initial (knowledge) state node for agent.
       (set new-state-node-name
            (make-state-node
               :terms new-terms
               :name new-state-node-name
       ...
       (setf (aref *event-queue* j) '())
       (setq *real-clock* 0); actually here there'll be no difference
                            ; between real and agent clocks; note that
                            ; they should just be incremented by 1 for
                            ; each iteration (thru all agents) in
                            ; 'run-multiple-agents'
       (setf (aref *AG-clock* j) 0)
       (setf (aref *total-value* j) 0)
       (setq *real-history* '()); this is just the combination of
                                ; histories, but we won't try to display
                                ; them in parallel groups of steps...
       (setf (aref *AG-history* j) '())
       (setf (aref *curr-state-node* j) 
             (eval new-state-node-name))
 )
); end of initialize-state-node

;example (maphash #'print-hash-entry *my-hash*)
...
; argument is the agent, even if the predicate is occluded.
;
  (let* ((objects (append (objects-colocated-with agent wff-htable)
                          (points-visible-to agent wff-htable)))
                   ;^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                   ; ABOVE ADDED FOR BRENNER-NEBEL MULTI-AGENT GRIDWORLD -LKS
         ;(aug-objects (cons (find-location agent *world-facts*) (cons agent objects)))
         			; ABOVE NEEDED FOR BRENNER-NEBEL MULTI-AGENT GRIDWORLD - Daphne
         			; if *roadmap-knowledge* is not all added in initialize-state-node
        ...
        ; We go through the entire hash table because we don't know
        ; what predicates we're looking for (but perhaps we should
        ; index facts via their first argument as well?) -comment by LKS
        (loop for value being the hash-values of wff-htable
        	 using (hash-key key)
	         do 	(setq wffs-at-curr-key 
                      (remove-if #'occluded-fact
                        (remove-if-not 
                          #'(lambda (w) (memberf (second w) aug-objects)) 
                            (cdr value)
                        )
                      )
                     )    	
                     (setq terms-evident-to 
	                     (merge_term_list_with_term_list  
	                        (add_list_of_tuples_to_hashtable
	                         wffs-at-curr-key evident-wff-htable to-collect-terms)
	                         terms-evident-to)
                     )
                     (setq list-of-evident-wffs 
                       (unionf wffs-at-curr-key list-of-evident-wffs))
       )
;(maphash #'print-hash-entry evident-wff-htable)       
       (cons evident-wff-htable 
          (list terms-evident-to list-of-evident-wffs))
  )
...
;
  (prog (loc key local-objects left-comoving-wffs right-comoving-wffs
       ...
       (setq left-comoving-wffs
          (apply 'append    
             (mapcar #'(lambda (x) (cdr x))  
               (mapcar #'(lambda (key) 
                          (gethash (list key) wff-htable)) *left-comoving-preds*)
             ) 
          )
       )
       ...
       (setq right-comoving-wffs
          (apply 'append    
             (mapcar #'(lambda (x) (cdr x))  
               (mapcar #'(lambda (key) 
                          (gethash (list key) wff-htable)) *right-comoving-preds*)
             ) 
          )
       )
  ...
  more (setq something-new nil)
       ...
       ; Oct 2/12: Modified to allow for different agents (and the old
       ; version seemed to incorrectly make the guru's local objects 
       ; available to AG as "visited objects")
       (setq j (gethash agent *agent-goal-indices*)); the index of the agent
       (setf (aref *visited-objects* j)
             (unionf (aref *visited-objects* j) local-objects))
...
(defun points-visible-to (agent wff-htable); Oct 5/12 -LKS
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
; FOR BRENNER-NEBEL MULTI-AGENT WORLD (OR OTHER WORLDS WITH UNKNOWN OBSTACLES)
; These are the 4 grid points vertically and horizontally adjacent to
; the current location of the agent; thus the agent will be able to tell
; if such a point is occupied or not, and thus can('nt) be moved to.
  (let* ((loc (find-location agent wff-htable))
         (adjacency-facts 
              (gethash `(is_adjacent_to ,loc nil) wff-htable)))
        (mapcar #'third (cdr adjacency-facts))))
...
(defun all-inferences (ground-facts-htable general-facts limit); Revised Dec. 2009 by Daphne Liu
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
(defun new-inferences (facts-htable rule-packets inferences-htable); Revised Dec. 2009 by Daphne Liu
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
; i.e., obtainable by matching all antecedents of a rule,
...
(defun graft-into-packet (facts-htable augm-packet); Revised Dec. 2009 by Daphne Liu
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
; facts-htable contains ground atomic wff's;
; augm-packet is of form (rule (g_1 ... g_m) f_1 ... f_n), with m=0 initially
; (for further details see `new-inferences' or `shake-packet'.)
;
; If a wff in facts-htable does not appear among g_1 ... g_m or f_1 ... f_n, 
; then if the wff unifies with some antecedent literal of the rule
; in the augm-packet, then graft the fact into the list occupying
; second position (i.e., right after the rule) in the double
; packet, provided that it isn't in that list, or in the cddr
; of the double packet yet; return the (possibly) altered packet;
;
(let* ((result augm-packet)
       (rule (car result))
       (goals (goals-of-rule rule)) 

       ; Oct 5/12: This assumes atomic antecedents, so I've replaced it. -LKS
       ; (wff-list (cdr (gethash (convert_pred_to_hashkey (car rule)) facts-htable))) 
       ; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
       (wff-list 
         (apply #'append 
           (mapcar #'(lambda (g) (cdr (gethash (convert_pred_to_hashkey g)
                                       facts-htable))) goals)))
      ) 
      (dolist (wff wff-list)
        (when (and (not (memberf wff (second result)))
              ...
              (setq result (cons rule (cons (cons wff (second result)) 
                                                  (cddr result))))
        )
      )
      result
...
(defun shake-packet (augm-packet inferences-htable); Revised Dec. 2009 by Daphne Liu
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
;       of such single-goal bindings found;
...
;       bindings2 := all (joint) bindings of the remaining 
;       goals (if any) to all-facts, and combine the binding u
...
; 3. Return `inferences'.
;
; (format t "~%AUGMENTED PACKET:~%  ~s~%" augm-packet); DEBUG
  (let* ((rule (car augm-packet))
       ...
       (dolist (u bindings)
          ...
          (dolist (u2 bindings2)
             (add_tuple_to_hashtable 
               (subst-unifier u2 (third rule)) inferences-htable 'NIL) 
          )
       )
       inferences-htable
  )
...
;
; Daphne: Revised 10/9/2012 to handle new representation of terms
(defun notice-new-local-facts (state-node agent)
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~; Oct 2/12: multiagent modif'ns -LKS
...
; - compute local-beliefs := (facts-evident-to agent wff-htable) 
; - delete from wff-htable those local-beliefs that are not local-facts
; - Compute local-facts := (facts-evident-to agent *world-facts*);
...
; *here*[agent-index] is also updated, just in case it's needed;
;
...
; locally observable.
;
  (let* ((j (gethash agent *agent-goal-indices*)) ; agent index
         (wff-htable (state-node-wff-htable state-node))
         (new-terms (state-node-terms state-node))
         ; beliefs that *should* be evident facts if true:
         (local-beliefs 
            (facts-evident-to agent wff-htable 'NIL)
         )
         ; actual local unoccluded facts:
         (local-facts 
            (facts-evident-to agent *world-facts* 'NIL)
         )
...
;     new-terms); DEBUG
        ...
        (setf (aref *here* j) (find-location agent wff-htable)); not sure if needed...
        
        (setf (aref *visited-places* j)
              (remove-duplicates (cons (aref *here* j) 
                                       (aref *visited-places* j))))
...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
; STUB: We don't allocate values to states, only to actions,
; in the colorball gridworld.
 0); end of state-value

     
(defun expected-rewards (wff-htable) ;(wffs)
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
; For the colorballs world this is a stub, returning a fixed value.
; I.e., we don't know how "promising" an unexpanded looks, in terms 
; of the action sequences (and thus state sequences) it can be the
; start of. The value should make no difference in the colorballs world.
;
  2
)
