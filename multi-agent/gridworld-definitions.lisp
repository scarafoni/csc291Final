...
;; 'is_adjacent_to' knowledge added for adjacent points, -LKS Sep 26/12 
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defparameter *n-agents* 4); Brenner & Nebel allowed up to 10
                                      
(defparameter *extra-initial-knowledge* (make-array *n-agents*))
(dotimes (j *n-agents*)
   (setf (aref *extra-initial-knowledge* j) (make-hash-table :test #'equal))) 
; Extra initial knowledge that may be supplied to an agent when the initial 
...
; knowledge normally is supplied as curr-facts in the place-obj command,  
; but the user can also explicitly add facts to *extra-initial-knowledge*
...
(defparameter *epistemic-preds* '(wants knows is_obligated_to believes))
...
(defparameter *visited-places* (make-array *n-agents*))
(defparameter *visited-objects* (make-array *n-agents*))
;(defparameter *roadmap-extrainitial-knowledge-terms* nil)   

(defparameter *real-clock* 0)
(defparameter *ag-clock* (make-array *n-agents*))

(defun def-roadmap (points roads); Revised Dec. 2009 by Daphne Liu
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~; Revised Sep 26/12 by LKS to provide
                                 ; adjacency knowledge for point locations
                                 ; (needed for this an other problems)
                                 ; NB: FOR THE MULTIAGENT WORLD,
                                 ; *BLOCKED-POINTS* MUST BE PREDIFINED
...
;    of a list of (road-name adjacent-point distance) triples; 
;    this can be done in an obvious way by sequentially processing
;    the given roads; 
; 	 
...
;
 (let (tuple name pt-dist-list pt-list dist-list curr trimmed-pt-list 
   ...
   (dotimes (j *n-agents*)
      (clrhash (aref *extra-initial-knowledge* j)))
   ...
   (dolist (r roads); error checking
	...
	(setq pt-list '())
   	...
   	(dotimes (i leng t)
		...
		(if (evenp i)
   			...
   			(progn 	
                                 (format t 
                                   "~%***Road ~a is missing a valid segment" name)
          	...
          	(if (numberp curr)
            	    (push curr dist-list)
   		    (progn (format t 
                             "~%***Road ~a is missing a valid distance" name)
            	           (return-from def-roadmap 'ERROR-TERMINATION) 
            	)
          	) 
		) 
	...
	(when (not (= (- (length pt-list) 1) (length dist-list)))
	      (format t 
                       "~%***Road ~a is missing a valid segment or distance" name)
	      (return-from def-roadmap 'ERROR-TERMINATION) 
	)
	
	(setq road-points (union pt-list road-points))
           	
	; Create a roadmap knowledge (for road r) for the Motivated Explorer (AG):
	(add_tuple_to_hashtable
                 (cons 'road (list name)) *roadmap-knowledge* 'NIL)
        ; NOT NEEDED FOR PRESENT (MULTIAGENT) DOMAIN
        ; (add_tuple_to_hashtable 
        ;         (cons 'navigable (list name)) *roadmap-knowledge* 'NIL)
	
        ; process first point on road r, then remaining points and
        ; distances in tandem:
	(setq pt1 (pop pt-list))
        ; NOT NEEDED FOR PRESENT (MULTIAGENT) DOMAIN:
	; (add_tuple_to_hashtable 
        ;     (list 'is_on pt1 name) *roadmap-knowledge* 'NIL)		
		
	(while (> (length pt-list) 0)
	       (setq pt2 (pop pt-list))
               ; NOT NEEDED FOR PRESENT (MULTIAGENT) DOMAIN
	       ; (add_tuple_to_hashtable 
               ;          (list 'is_on pt2 name) *roadmap-knowledge* 'NIL)	
	       (setq currleng (pop dist-list))
	       (push (list name pt2 currleng) (get pt1 'next))
               ; NEEDED FOR PRESENT (MULTIAGENT) DOMAIN:
               (add_tuple_to_hashtable
                 (list 'is_adjacent_to pt1 pt2) *roadmap-knowledge* 'NIL)
	       (push (list name pt1 currleng) (get pt2 'next)) 
               (add_tuple_to_hashtable
                 (list 'is_adjacent_to pt2 pt1) *roadmap-knowledge* 'NIL)
	       (setq pt1 pt2)
	)
   )
   ...
   ; LKS, 10/1/12: FOR THE MULTIAGENT PROBLEM, MAKE THE BLOCKED LOCATIONS
   ; 'OCCUPIED', AS PART OF INITIAL *PROTECTED-FACTS* (TO BE UPDATED BY AGENT
   ; PLACEMENT AND MOVES ONLY, NOT BY INFERENCE RULES):
   (dolist (p *blocked-points*)
           (add_tuple_to_hashtable (list 'is_occupied p) *protected-facts* nil)
           (add_tuple_to_hashtable (list 'is_occupied p) *world-facts* nil))

   ; THIS SHOULD BE IN "gridworld-world.lisp". -LKS
...
;                    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~; Revised to add 'is_occupied'
;                                                ; facts for agents placed at 
;                                                ; points. -LKS
...
; the present version of the code just to set *AG-clock*j], not 
; in any predicates (time remains implicit in predicates),
;
  (let (j facts list-of-terms-in-added-tuple)
    ; type predication about 'name':
    (push (list obj-type name) facts) 
    (push (list 'is_at name point) facts)
    ; ADDED BY LKS (TO REFLECT GRID POINTS GETTING BLOCKED BY AGENTS):
    (when (eql obj-type 'agent)
          (add_tuple_to_hashtable (list 'is_occupied point) 
              *protected-facts* nil)
          (add_tuple_to_hashtable (list 'is_occupied point)
              *world-facts* nil)); END OF ADDITION
    ; [Oct 2/12: changed to allow an array of *here*'s and agent clocks]
    ; set global *here*[j] and *AG-clock*[j] parameters if name is 'name':
    (when (member name *agent-names*)
          (setq j (gethash name *agent-goal-indices*))
          (setf (aref *here* j) point)
          (setf (aref *ag-clock* j) time-pt))
    ; The above replaces
    ...
    (dolist (f (remove-duplicates curr-facts :test #'equal)) 
       (push f facts)
       (when j ; if j is non-nil, it is the agent index for 'name' (set above)
         (if (not (eq (second f) name)); not self-knowledge (which will be
                                       ; available w/o explicit belief-storage)?
             (add_tuple_to_hashtable f 
               (aref *extra-initial-knowledge* j) 'NIL))
       )
    )
...
;(maphash #'print-hash-entry *roadmap-knowledge*)
