; I'm separating out this function because it sometimes causes compiler errors.
; -LKS

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; This procedure executes the first action specified by arg action-name 
;; of the best plan found in go!, updates *real-clock* and *AG-clock*[j], 
;; and refreshes the actual new current state *curr-state-node*[j] after 
...
;; *real-clock* and *AG-clock*[j] is logged in *real-history* and 
;; *AG-history*[j], respectively.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
...
(defun implement-effects (action-name agent)
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
 (let*	((j (gethash agent *agent-goal-indices*))
         (the-action (action-type action-name))
         (action (eval action-name)); the agent's action concept (op. instance)
        ...
        (setf (aref *total-value* j)
           (incf (aref *total-value* j) 
                 (+ (op-value action) 
                    (state-node-local-value (aref *curr-state-node* j)))))
        ...
        (while (or (eq 'T is-first-iter) (eq is-terminated 'NIL))
           ...
           (if (eq 'T is-first-iter)
               ...
               (progn 
                  ;(handleExtOps)
                  ; Above line commented out only for opp'c (i.e., normal) runs
           ...
           (when (not is-terminated)
               ...
               (setq bindings (remove-if #'degenerate-binding 
                               (all-bindings-of-goals-to-fact-htable deletes
                                 ...
                                 (state-node-terms (aref *curr-state-node* j)))))
               ...
               (setq new-terms (state-node-terms (aref *curr-state-node* j)))
               (setq new-wff-htable (state-node-wff-htable (aref *curr-state-node* j)))
               (setq new-terms 
                 (remove_term_list_from_term_list 
                   (remove_list_of_tuples_from_hashtable deletes new-wff-htable 'T)
                      new-terms))
               (setq new-terms 
                   (merge_term_list_with_term_list (add_list_of_tuples_to_hashtable adds new-wff-htable 'T)
                   new-terms))
               (setf (state-node-terms (aref *curr-state-node* j)) new-terms)
							
               ; Modify *curr-state-node*[j] to update the agent's beliefs 
               ...
               ; duration, and *AG-clock*[j] is logged in *AG-history*.  Only the 
               ; start and (later) the end, and not each iteration of execution, 
               ; of the action will be logged in *AG-history*[j] to reflect that 
               ; AG does not know in advance what the real duration of an action
               ; will surely be.
               (if (= 1 *event-timer*)
                   (push (list (cons the-action new-state) 
                               *event-timer* expected-duration (aref *AG-clock* j)) 
                         (aref *AG-history* j))) 
           )
           ; Increment *real-clock* and *AG-clock*[j] after each iteration of 
           ; execution.
           (if (eql j 0); for 1st agent only -- concurrent simulation
               (incf *real-clock* 1))
           (incf (aref *AG-clock* j) 1)
        ); end of executing the action iteration by iteration

        ; Log the end of the action in *AG-history*[j] to indicate AG's awareness 
        ; of its termination.
        (when (> *event-timer* 2)
            (push (list (cons the-action new-state) (- *event-timer* 1) 
                        expected-duration 
                        (- (aref *AG-clock* j) 2)) (aref *AG-history* j))
        )
 )
